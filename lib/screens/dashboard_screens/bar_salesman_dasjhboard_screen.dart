...
import 'package:san_sprito/bloc/bar_salesman_dashboard/bar_salesman_dashbard_bloc.dart';
...
class BarSalesmanDashboardScreen extends StatefulWidget {
  final String? shopId;
...
class _SalesmanStockDashboardState extends State<BarSalesmanDashboardScreen> {
  ...
  BarSalesmanDashboardData? barStockData;
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<BarSalesmanDashboardBloc, BarSalesmanDashboardState>(
      listener: (context, state) {
        if (state is BarSalesmanDashboardLoading) {
          isLoad = true;
        } else if (state is UploadBarImageLoading) {
          isImageLoading = true;
        } else if (state is SaveBarStockLoadingState) {
          btnLoading = true;
        } else if (state is BarSalesmanDashBoardSuccess) {
          barStockData = state.barSalesmanDashBoardDataResponse.data;
          categories = barStockData?.categories ?? [];
          mockData = List.generate(barStockData?.stocks?.length ?? 0, (index) {
            var item = barStockData?.stocks?[index];
            return {
              "id": item?.id ?? "",
              "shop": item?.shopName ?? "",
          ...
          isLoad = false;
        } else if (state is GetBarBrandProductListSuccess) {
          _searchController.clear();
          getProductBrandList = state.getBrandProductResponseModel.data;
          isLoad = false;
        } else if (state is BarSaveStockSuccess) {
          getUserIdAndLoadData();
          isLoad = true;
          btnLoading = false;
        ...
        } else if (state is UpdateBarStockSuccess) {
          _searchController.clear();
          isLoad = true;
          btnLoading = false;
          getUserIdAndLoadData();
          ...
          emptyFields();
        } else if (state is DeleteBarStockSuccess) {
          _searchController.clear();
          Navigator.pop(context);
          isLoad = true;
          ToastService.showInfo("Stock Deleted Successfully");
          getUserIdAndLoadData();
        } else if (state is UploadBarImageSuccess) {
          isImageLoading = false;
          ToastService.showSuccess("Image Uploaded Successfully");
          imageNames.clear();
          imageFiles.clear();
        } else if (state is BarSalesmanFailure) {
          _searchController.clear();
